...
if (!$dataCompleted) {
    ...
    foreach ($items as $item) {
        $banner = [
            ...
            'url' => $item['ban_link'],
            'object' => [
                'type' => $item->ban_object_type,
                'id' => $item->ban_object_id
            ],
        ...
        if ($item->ban_object_type && $item->ban_object_id)
            switch ($item->ban_object_type) {
                ...
                case 'TOP_RACING':
                    $item->banner->topRacing = $item->banner->topRacing ?? [];
                    if ($item->banner->topRacing) {
                        $banner['top_racing'] = transformer_item($item->banner->topRacing, new \App\Transformers\TopRacingCampaignTransformer(), ['products']);
                    }
