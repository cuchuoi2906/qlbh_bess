...
$ord_code = getValue('ord_code', 'str', 'GET', '');
if ($ord_code) {
    $items_model->where('ord_code', $ord_code);
}

//Lấy tổng số lượng bán hàng theo sản phẩm
$items = $items_model->fields('pro_id, pro_name_vn, pro_code, COUNT(opc_order_id) AS total_count, ord_created_at, ord_code')
...
foreach ($items as $item) {
    $item->total_quantity = $item->total_quantity / $item->total_count;
...
$dataGrid->column('ord_created_at', 'Ngày', 'string');
