...
if (
    ...
    if ($product) {
        $old_price = $product->orp_sale_price;
        $product->orp_sale_price = $price;
        $product->update();
        //Tính lại hoa hồng
        ...
        \App\Models\OrderLog::insert([
            ...
            'orl_note' => 'Thay đổi số lượng sản phẩm ' . $product->info->name . ' từ ' . $old_price . ' thành ' . $price
        ]);

        admin_log($admin_id, ADMIN_LOG_ACTION_EDIT, $order->id, 'Đã thay đổi gíá sản phẩm ' . $product->name . ' của đơn hàng (' . $order->id . ') từ ' . $old_quantity . ' thành ' . $quantity);
    }
}