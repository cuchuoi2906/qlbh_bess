...
$dataGrid->column('ban_image', 'Ảnh', function ($row) {
    if ($row['ban_image']) {
        return '<img src="http://' . $_SERVER['HTTP_HOST'] . '/upload/banners/' . $row['ban_image'] . '" />';
    }
...
$dataGrid->column('ban_create_time', 'Ngày tạo', ['string|center', function ($item) {
    return date('d/m/Y H:i', $item->create_time);
}]);
$dataGrid->column('ban_update_time', 'Ngày cập nhật', ['string|center', function ($item) {
    return ($item->update_time != null && $item->update_time > 0) ? date('d/m/Y H:i', $item->update_time) : '--';
}]);
